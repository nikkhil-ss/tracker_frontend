...
export class AdminviewComponent implements OnInit {
  ...
  isEditing(candidate: Interview): boolean {
    return this.editedCandidates.includes(candidate);
  }

  selectAllCandidates(checked: boolean) {
    this.filteredCandidates.forEach(
      (candidate) => (candidate.isSelected = checked)
    );
  ...
  updateSelectedCandidates() {
    this.selectedCandidates = this.filteredCandidates.filter(
